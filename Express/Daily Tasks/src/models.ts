const queries = {
	select: {
		from_user_name_and_pass_word: `SELECT * FROM users WHERE user_name=? AND pass_word=?`,
		fromUserId: `SELECT * FROM users WHERE user_id=?`,
		fromUserName: `SELECT * FROM users WHERE user_name=?`,
		fromUserIdBasic: `SELECT user_id,user_name,first_name,last_name,extension FROM users WHERE user_id=?`,
		fromUserNameBasic: `SELECT user_id,user_name,first_name,last_name,extension FROM users WHERE user_name=?`,
		search: `SELECT user_id, user_name AS conv_name, title, icon FROM users WHERE user_name LIKE ? OR title LIKE ?`
	},
	insert: `INSERT INTO users(title, user_name, pass_word) VALUES(?, ?, ?)`,
